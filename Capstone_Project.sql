...
--B)Çalışanların yıllara göre satış performans değişimi
...
case
when extract(year from o.order_date) = 1996 and extract(month from o.order_date) >= 7 then 1996
when extract(year from o.order_date) = 1998 and extract(month from o.order_date) <= 5 then 1998
else extract(year from o.order_date)
end as adjusted_year,
sum(od.unit_price * od.quantity * (1 - od.discount)) AS total_sales
from orders as o
join order_details od on o.order_id = od.order_id
join employees as e on o.employee_id = e.employee_id
where o.order_date between '1996-07-01' and '1998-05-31'
group by e.employee_id, employee_name, adjusted_year
)
select 
current_year.employee_name,
current_year.adjusted_year as year,
current_year.total_sales as current_year_sales,
previous_year.total_sales as previous_year_sales,
((current_year.total_sales - previous_year.total_sales) / previous_year.total_sales) * 100 AS percentage_change
from sales_per_year current_year
left join sales_per_year previous_year 
on current_year.employee_id = previous_year.employee_id
and current_year.adjusted_year = previous_year.adjusted_year + 1
order by current_year.employee_name, current_year.adjusted_year;
...
select cu.customer_id,contact_name,country, count(o.order_id) as order_count,
...
--B)Müşterilerin en fala sipariş ettiği 2 ürün
...
row_number() over (partition by cu.contact_name order by sum(od.quantity) desc) as rank
...
FROM 
    customers AS cu
JOIN 
    orders AS o ON o.customer_id = cu.customer_id
JOIN 
...
with avg_ord as(select cu.customer_id, count(o.order_id) as order_count,round(sum(unit_price * quantity*(1-discount)):: numeric,2) as total_sales, max(order_date) 
from customers as cu
join orders as o on o.customer_id=cu.customer_id
join order_details as od on od.order_id=o.order_id
group by 1
...
with avg_sales as(select cu.customer_id, count(o.order_id) as order_count,
round(sum(unit_price * quantity*(1-discount)):: numeric,2) as total_sales, max(order_date) 
from customers as cu
join orders as o on o.customer_id=cu.customer_id
join order_details as od on od.order_id=o.order_id
group by 1
...
--3)Ürün Kategorisi ve Satış Analizi:

--A) Kategori bazında toplam satış miktarı ve geliri. 
...
--A)genel ürün satış ve toplamı analizi

select 
product_name, 
ROUND(sum((od.unit_price * od.quantity * (1 - discount))::numeric), 2) AS total_sales,
sum(od.quantity) as  total_order_count
from orders as o
...
--B) ürünlerin fiyat aralığı ve ne kadar satıldığı

select product_name, 
case when pr.unit_price < 10 then '0-10'
when pr.unit_price >= 10 and pr.unit_price <30 then '10-30'
when pr.unit_price >=30 and pr.unit_price <50 then '30-50'
when pr.unit_price >=50 and pr.unit_price <70 then '50-70'
when pr.unit_price >=70 and pr.unit_price <100 then '700-100'
when pr.unit_price >=100 then 'over 100'
else 'fiyat yok' end as price_range,
sum(od.quantity) as product_sold_count
from products as pr
...
---c) stok durumlarına göre anliz	
	
select product_name,
case when unit_in_stock > reorder_level then 'stok yeterli'
when unit_in_stock = reorder_level then 'stok sınırda'
when unit_in_stock <= reorder_level then'stok az'
else 'stok yok' end as stok_durumu,
...
--A)Yıllık Ciro Analizi
with sales_per_year as (
select 
case
when extract(year from o.order_date) = 1996 and extract(month from o.order_date) >= 7 then 1996
when extract (year from o.order_date) = 1998  and extract (month from o.order_date) <= 5 then 1998
else extract(year from o.order_date) end as adjusted_year,
sum(od.unit_price * od.quantity * (1 - od.discount)) as total_sales
from orders as o
join order_details od on o.order_id = od.order_id
where o.order_date between '1996-07-01' and '1998-05-31'
...
from sales_per_year current_year
left join
sales_per_year previous_year on current_year.adjusted_year = previous_year.adjusted_year + 1
order by 1;


---B)Aylık ciro değişimi
select
TO_CHAR(order_date, 'MM') AS month,
sum(case when TO_CHAR(order_date, 'YYYY')::integer = 1996
then ROUND(cast(od.unit_price * od.quantity * (1 - od.discount) AS numeric), 2) else 0 end ) AS sales_1996,
sum(case when TO_CHAR(order_date, 'YYYY')::integer = 1997
then ROUND(cast(od.unit_price * od.quantity * (1 - od.discount) AS numeric), 2) else 0 end) AS sales_1997,
sum(case when TO_CHAR(order_date, 'YYYY')::integer = 1998
then ROUND(cast(od.unit_price * od.quantity * (1 - od.discount) AS numeric), 2) else 0 end) AS sales_1998
from orders AS o 
...
--C)Aylık miktar değişimi

select
TO_CHAR(order_date, 'MM') AS month,
sum(case when TO_CHAR(order_date, 'YYYY')::integer = 1996
then od.quantity else 0 end) AS quantity_1996,
sum(case when TO_CHAR(order_date, 'YYYY')::integer = 1997
then od.quantity else 0 end) AS quantity_1997,
sum(case when TO_CHAR(order_date, 'YYYY')::integer = 1998
then od.quantity else 0 end) AS quantity_1998
from orders AS o 
join order_details AS od 
ON o.order_id = od.order_id
GROUP BY 1
...
--6) Ülkelere göre satış analizi
	
--A)Ülke bazlı satış miktar ve satış toplam analizi

select ship_country,count(distinct o.order_id) as order_count, 
round(sum(odet.unit_price * quantity*(1-discount)):: numeric,2) as total_sales,
round(AVG(shipped_date::date - order_date::date),2) AS average_shipping_time
from orders as o 
join customers as cu on o.customer_id=cu.customer_id
join order_details as odet on o.order_id=odet.order_id
join products as pr on odet.product_id=pr.product_id
group by 1
ORDER BY 4 DESC


---B) Ülkelere göre en çok satılan ürün
	
with ranked as (select ship_country,
product_name,
sum(od.quantity) as total_quantity,
row_number() over(partition by ship_country order by sum(od.quantity) desc) as rank
from order_details as od
join orders as o on od.order_id = o.order_id
join customers as cu on cu.customer_id = o.customer_id
join products as p on p.product_id=od.product_id
